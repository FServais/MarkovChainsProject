...
\paragraph{3)} Nous avons choisi un nombre de pas $t = 20$. Le cas où le surfeur démarre aléatoirement sur le graphe est représenté par une distribution initiale $\pi_0$ uniforme et le cas où il démarre d'une page fixe est représenté par une distribution initiale $\pi_0$ où toutes les probabilités sont nulles sauf celle située sur l'index correspondant au nœud de départ. L'évolution des probabilités dans les deux cas est donnée sur la Figure \ref{fig:q113}.
\begin{figure}[h]
	\center
...
\end{pmatrix}
\]
Si on observe les matrices $Q^{(k)}$ pour $k> 20$, on peut constater que les éléments se stabilisent et que les lignes s'égalent. 
...
\begin{itemize}
	...
	\item les \textbf{noeuds 2} et \textbf{4} possèdent le même nombre intermédiaire (par rapport aux deux autres) d'arêtes entrantes. Le PageRank du noeud 4 est néanmoins plus élevé que celui du noeud 2 puisque le noeud 4 possède une arête entrante venant du noeud 1 qui est le plus visité.
	\item malgré un nombre d'arêtes entrantes plus élevé que le noeud 3, le \textbf{noeud 2} possède une PageRank égale. Cela est dû au fait que, d'une part, le noeud 3 peut être visité depuis le noeud le plus visité (noeud 1) ce qui améliore son PageRank et, d'autre part, que le noeud 2 ne peut être accédé que depuis des noeuds moins visités (noeud 3 et 4) ce qui abaisse son PageRank. 
\end{itemize} 
...
\paragraph{1)} Pour pouvoir calculer des éventuelles distribution stationnaires sur base des matrices $A_2$ et $A_3$, nous avons calculés les matrices de transition $Q_2$ et $Q_3$ : 
\[
Q_2 = 
\begin{pmatrix}
         0 &   1.0000  &       0  &       0 \\
...
\end{pmatrix}
\]
Nous avons ensuite appliqué la méthode des puissances pour obtenir des distributions $\pi^{(k)}$ successives. Le résultat est donné sur la Figure \ref{fig:q118}. On constate qu'aucune distribution stationnaire n'a pu être trouvée étant donné la présence d'\textbf{oscillations}.
\paragraph{}
...
\paragraph{2)}
Nous avons analysé l'évolution du PageRank de certaines pages lorsque alpha évolue. En regardant la figure (JE NE SAIS PAS CMT FAIRE DES REFERENCES :'( ) , on observe que plus alpha est grand, plus le PageRank des pages tend à s'uniformiser. En effet, un PageRank élevé au départ va diminuer avec l'augmentation de $\alpha$, tandis qu'un PageRank assez bas va augmenter avec $\alpha$.   
\begin{figure}[h]
	\center
	\includegraphics[scale=0.5]{../images/evolPageRankAlpha.png}
	
	\caption{Évolution du PageRank avec $\alpha$}
\end{figure} 

\paragraph{3)}
Nous avons déterminé l'évolution du nombre d'itérations nécessaires pour trouver une distribution stationnaire lorsque $\alpha$ varie. Les résultats sont présentés sur la Figure suivante (HELP REF) : 
\begin{figure}[h]
	\center
	\includegraphics[scale=0.5]{../images/nbItAlph.png}
	
	\caption{Effet de $\alpha$ sur la vitesse de convergence}
\end{figure} 
...
\begin{itemize}
	...
	\[
		...
		\end{pmatrix}
	\]
	\item $N$, la matrice dont l'élément $\alpha_{ij}$ est le nombre de transition de l'état $i$ à $j$ dans la trace $X$
	...
	\[
		P(X|\underline{\theta}_i) = \theta_{i1}^{\alpha_{i1}} \times \cdots \times \theta_{i(n - 1)}^{\alpha_{i(n - 1)}} \times \left(1 - \sum\limits_{k = 1}^{n - 1} \theta_{ik}\right)^{\alpha_{in}} = \left(1 - \sum\limits_{k = 1}^{n - 1} \theta_{ik}\right)^{\alpha_{in}} \times \prod\limits_{k = 1}^{n - 1} \theta_{ik}^{\alpha_{ik}}
	\]
...
\[
	\begin{aligned}
		S_{ij} &= \left[\alpha_{ij} \theta_{ij}^{\alpha_{ij} - 1} \left(1 - \sum\limits_{k = 1}^{n - 1} \theta_{ik}\right)^{\alpha_{in}} - \alpha_{in} \left(1 - \sum\limits_{k = 1}^{n - 1} \theta_{ik}\right)^{\alpha_{in} - 1} \theta_{ij}^{\alpha_{ij}}\right] \times \prod\limits_{k = 1, k \neq j}^{n - 1} \theta_{ik}^{\alpha_{ik}}\\
		&= \left[\alpha_{ij}  \left(1 - \sum\limits_{k = 1}^{n - 1} \theta_{ik}\right) - \alpha_{in}  \theta_{ij}\right]\times \left(1 - \sum\limits_{k = 1}^{n - 1} \theta_{ik}\right)^{\alpha_{in} - 1} \times \theta_{ij}^{\alpha_{ij} - 1} \times \prod\limits_{k = 1, k \neq j}^{n - 1} \theta_{ik}^{\alpha_{ik}} = 0\\
	\end{aligned}
\]
Il nous reste à résoudre l'équation suivante :
\[
	\begin{aligned}
		&\alpha_{ij} \left(1 - \sum\limits_{k = 1}^{n - 1} \theta_{ik}\right) - \alpha_{in} \theta_{ij} = 0\\
		\Leftrightarrow \text{ }&\alpha_{ij} \left(1 - \sum\limits_{k = 1}^{n - 1} \theta_{ik}\right) = \alpha_{in} \theta_{ij}\\
		\Leftrightarrow \text{ }& \dfrac{\alpha_{in}}{\alpha_{ij}} \theta_{ij} + \sum\limits_{k = 1}^{n - 1} \theta_{ik} = 1\\
		\Leftrightarrow \text{ }& \left(\dfrac{\alpha_{in}}{\alpha_{ij}} + 1\right) \theta_{ij} + \sum\limits_{k = 1, k \neq j}^{n - 1} \theta_{ik} = 1		
	\end{aligned}
\]
...
S_i \equiv 
\begin{pmatrix}
\left(\dfrac{\alpha_{in} + \alpha_{i1}}{\alpha_{i1}}\right) & 1 & \cdots & 1 \\
1 & \ddots &  \ddots & \vdots \\
\vdots &  \ddots & \ddots & 1\\
1 & \cdots & 1   & \left(\dfrac{\alpha_{in} + \alpha_{i(n - 1)}}{\alpha_{i(n - 1)}}\right)
\end{pmatrix}
\begin{pmatrix}
\theta_{i1}\\
\vdots\\
...
\end{pmatrix}
\]
La résolution de ce système pour chaque ligne permet d'obtenir une estimation de la matrice de transition. 
\paragraph{}
Nous avons implémenté cette solution et les tests se sont révélés concluants. Il reste néanmoins un problème à régler. En effet, dans le système nous divisons par le nombre de transition $\alpha_{ij}$ qui pourrait être nul si la transition de l'état $i$ à $j$ n'a pas été observée dans la trace étudiée. Il faut donc appliquer un traitement à la matrice $N$ afin de supprimer les valeurs nulles.
\paragraph{}
...
Nous avons implémenté un algorithme utilisant cette solution. Il se trouve dans le Listing \ref{lst:estQ_1}. 
\begin{center}
\begin{minipage}{1.1\linewidth}
\begin{lstlisting}[caption = {Calcul de la matrice de transition 1}, label = {lst:estQ_1}]
function Q = estimateQ(X,k)
	% Q : the estimated transition matrix
	% X : the markov chain
	% k : the number of possible states of the markov chain
	
\end{lstlisting}
\end{minipage}
\end{center}
\subsection{Origine d'une trace}
Notons $tr$ une trace dont on souhaite définir l'origine. Soit $Q_i$ la variable aléatoire qui représente l'appartenance à un surfeur i . On souhaite définir la probabilité d'appartenance d'une trace à un surfeur: 
$$ P(Q_i |tr) = \dfrac{P(tr|Q_i) \cdot P(Q_i)}{P(tr)}$$ On obtient ce résultat grâce à la formule de Bayes. D'une part, on connait $P(tr|Q_i)$ qui est la probabilité que le surfeur i effectue la trace $tr$ étant donné la matrice de transition $Q$ donnée par la méthode exposée au poin précédant. Ensuite, la probabilité d'appartenir au surfeur 1 ou 2 vaut 0.5 dans ce cas-ci étant donné que nous sommes certains que la moitié des traces appartient au surfeur 1 et l'autre au surfeur 2.
On peut développer le terme $P(tr)$ :
$$ P(tr) = $$
Étant donné que les probabilités sont infimes, on utilise la fonction log. \\ \\
Afin de définir l'appartenance d'une trace au surfeur 1 ou 2, on va calculer le rapport :$$ log (\dfrac{P(Q_1|tr)}{P(Q_1|tr)})$$ Si ce rapport est positif, cela signifie que la fraction est supérieure à 1 et donc que la trace a été réalisée par le surfeur 1. Dans le cas contraire, elle appartient au surfeur 2.
\subsection{Origine des traces}
\subsection{Hypothèses, fiabilité et limitations}

\subsection{Applications}
Cette démarche a de nombreuses applications possibles dans des domaines très variés. Nous avons implémenté ce système pour définir l'appartenance d'un texte à un ensemble : par exemple, on peut définir si un texte a été écrit par une personne ou l'autre, si ce texte est en français, anglais, si un code est en langage C ou Java, et bien d'autres sur base d'un échantillon d'un groupe. 
