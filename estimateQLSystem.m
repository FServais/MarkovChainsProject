function Qest = estimateQLSystem(Tr,k)
%ESTIMATEQ estimate the transition matrix of a markov chain
%   Q = ESTIMATEQ(Tr,k) : if Tr is a transition matrix and k the number of 
%   possible state, then Q is the transition matrix estimated using the 
%   maximum likelihood method (by solving the linear system)
	B = ones(k - 1, 1);
	Q = zeros(k,k);
