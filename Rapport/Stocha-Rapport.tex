...
\begin{itemize}
	\item[Méthode de Laplace (\textit{Laplace smoothing}) :] Cette méthode consiste à ajouter 1 à tous les éléments comptés. On obtient alors une probabilité de la forme : 
	\[
		P_{Laplace} (\omega_i | \omega_{i-1}) = \frac{c(\omega_{i-1}, \omega_i) + 1}{c(\omega_{i-1}) + n}
	\]
	\item[\textit{Add-k smoothing} :] Cette méthode généralise la méthode de Laplace où, cette fois, $k$ est ajouté au numérateur et $k.n$ est ajouté au dénominateur : 
	\[
		P_{Add-k} (\omega_i | \omega_{i-1}) = \frac{c(\omega_{i-1}, \omega_i) + k}{c(\omega_{i-1}) + k.n}
	\]
	\item[\textit{Unigram prior smoothing} :] Il s'agit d'une variante de l'\textit{add-k smoothing} où la probabilité d'obtenir l'unigramme $\omega_i$. Par rapport à la formule de l'\textit{add-k smoothing}, on prend $k = m/n$ où $m$ est un paramètre à déterminer. On obtient alors : 
	\[
		P_{Add-k-var} (\omega_i | \omega_{i-1}) = \frac{c(\omega_{i-1}, \omega_i) + m . \left(\frac{1}{n}\right)}{c(\omega_{i-1}) + m}
	\]
	La ``subtilité'' de l'\textit{unigram prior smoothing} consiste en remplaçant la constante $\frac{1}{n}$ au numérateur par $P(\omega_i)$. On a au final : 
	\[
		P_{UnigramPrior} (\omega_i | \omega_{i-1}) = \frac{c(\omega_{i-1}, \omega_i) + m . P(\omega_i)}{c(\omega_{i-1}) + m}
	\]
\end{itemize}
